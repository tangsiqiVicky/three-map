    ...
    parse: function ( text ) {
        ...
        function parseNode( node, style ) {
            ...
            switch ( node.nodeName ) {
                ...
                case 'svg':
                    //---------------自己编写------------
                    viewBoxW=node.viewBox.baseVal.width;
                    viewBoxH=node.viewBox.baseVal.height;

                    break;

                case 'g':
                    //console.log(node.childNodes[1].className.animVal)
                ...
                case 'style':
                    ...
                    //空对象
                    var newclass={}
                    // 节点下面的节点值：cssStyle样式表
                    var childrenNodeValue=node.childNodes[0].nodeValue;
                    // 分割以“}”字符串
                    var styleLenght=childrenNodeValue.split(/\}/gi);
                    //遍历分割后对象的个
                    for(var i=0;i<styleLenght.length;i++){
                        //查找包含.st字符串的对象
                        if(styleLenght[i].match(/.st\d+/gi)){
                            //存在fill填充的保留
                            if(styleLenght[i].match(/fill\:\#[0-9a-z]*/gi)){
                                //newclassStyle对象属性是：样式表中className，属性值：颜色值
                                newclass[styleLenght[i].match(/st\d+/gi)[0]]=styleLenght[i].match(/fill\:\#[0-9a-z]*/gi)[0].replace(/fill\:/,"")
                            }
                        }
                    }
                    //传入newclassStyle，全局可用，否则parseStyle()，不能调用
                    newclassStyle=newclass;
                    break;

                case 'text':
                    // 节点下面的节点值：文本内容
                    var childrenNodeValue=node.childNodes[0].nodeValue;
                    //先获取transform的值，然后用match用正则方法查找带小数点的数值
                    //newText[childrenNodeValue]=node.getAttribute("transform").match(/-?\d+\.?\d+/g);
                    //先获取transform的值，然后用split正则方法分割空格，去第4/5个，第5个去除右括号
                    newText[childrenNodeValue]=[node.getAttribute("transform").split(/\s/g)[4],node.getAttribute("transform").split(/\s/g)[5].replace(")","")];
                    newTextCont=newText;
                    break;

                case 'image':
                    // 节点下面的节点值：cssStyle样式表
                    var imgPngSplit=node.getAttribute("xlink:href").split(/\//g);
                    var imgPngName=imgPngSplit[imgPngSplit.length-1].replace(".png","");
                    if(newImg[imgPngName]){
                        numI++;
                        imgPngName=imgPngName+"more"+numI
                        newImg[imgPngName]
                    }
                    newImg[imgPngName]=[node.getAttribute("transform").split(/\s/g)[4],node.getAttribute("transform").split(/\s/g)[5].replace(")","")]
                    newImgCont=newImg;
            ...
            if ( path ) {
                ...
                paths.push( path );
                // 如果path路径存在id，就用id做类名，没有就ShapePath+t做类名
                if(node.getAttribute("id")){
                    pathsObj[node.getAttribute("id")]=path;
                }else{
                    pathsObj["ShapePath"+t]=path;
                    t++
                }
            ...
            if(node.childNodes.length){
                ...
                for (var i = 0; i<node.childNodes.length; i++) {
                    ...
                    if(node.childNodes[i].className){
                        //样式值：newclassStyle[子节点className]
                        style.fill=newclassStyle[node.childNodes[i].className.animVal]
                    }
                }
            }
            return style;
